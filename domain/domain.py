...
class Domain(object):
    ...
    def __init__(self, source=None):
        self.source = source
        self.load()
        # self.rpc_servers = self.get_rpcs('rpc_servers')

    # FIXME CACHE
    def get_rpcs(self, key=None, app_code="comistxs"):
        """获取应用的配置 默认是平台"""
        ...
        v = {}
        with get_connection('comistxs') as db:
            v = db.select_one(
                table='kv_config',
                where={'app_code': app_code, 'key': key},
            ) or {}
        if not v:
            return None
    ...
    def update(self, by=None, **kw):
        with get_connection(self.dbname) as db:
            db.update(self.table, kw, where=by)
    ...
    def _client(self, rpc):
        if not getattr(self, 'rpc_servers', None):
            self.rpc_servers = self.get_rpcs('rpc_servers')
        assert rpc in self.rpc_servers, ParamError('rpc error')
        conf = self.rpc_servers[rpc]
...
def drive(method, data):
    """调用其他系统dativer接口"""

    d = Domain()
    d.dbname = data['namespace']
    d.table = data['object']
    return d.rcall(data['namespace'], f'dativer.{method}', data)
