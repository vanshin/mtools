...
class PyMySQLCKConnection (MySQLConnection):
    type = "pymysqlck"
    ...
    def dict2sql(self, d, sp=','):
        ...
        for k,v in d.items():
            # k, v:
            # log_time ('between', ['2022-08-22 00:00:00', '2022-08-25 09:39:47'])
            k = self.key2sql(k)
            if isinstance(v, (tuple,list)):
                # 如果 like 传入列表，那么 使用 and 进行查询
                if isinstance(v[1], list) and v[0] == 'like':
                    for vv in v[1]:
                        x.append('%s' % self.exp2sql(k, v[0], vv))
                else:
                    x.append('%s' % self.exp2sql(k, v[0], v[1]))
            else:
                x.append('`%s`=%s' % (k.replace('.','`.`'),
            ...
            # x:
            # [
            #     "(`log_time` between  '2022-08-22 00:00:00' and '2022-08-25 09:39:47')",
            #     "(`log_level` in ('I','W','E'))",
            #     "(`record_str` like '%select%')",
            #     "(`record_str` like '%from%')"
            # ]
        return sp.join(x)
    ...
    def connect(self):
        ...
        log.info(
            'server=%s|func=connect|id=%d|name=%s|user=%s|role=%s|addr=%s:%d|db=%s',
            self.type, self.conn_id%10000,
            self.name, self.param.get('user',''), self.role,
            self.param.get('host',''), self.param.get('port',0),
            self.param.get('db','')
